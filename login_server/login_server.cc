...
int send_packet(login_client *client, void *data, int len);
 ...
 */
bool send_bb_security(login_client* client, uint32_t error) {
    bb_security_pkt pkt;
    ...
    pkt.team_id = dist(rand_gen) << 16 | dist(rand_gen);
...
int verify_account(login_client *client, bb_login_pkt *pkt) {
    ...
    if (!(result_row = mysql_fetch_row(result))) {
        ...
        send_bb_security(client, BB_LOGIN_ERROR_INVALID);
        return 1;
    }
    ...
    if (strcmp(password_hash, result_row[1])) {
        mysql_free_result(result);
        send_bb_client_message(client, "Username or password is incorrect. ");
        send_bb_security(client, BB_LOGIN_ERROR_LOCKED);
        return 1;
    }
    ...
    if (strcmp(result_row[8], "0")) {
        ...
        send_bb_security(client, BB_LOGIN_ERROR_BANNED);
        return 2;
    }
    // Inactive?
    ...
    for (c = clients.begin(), c_end = clients.end(); c != c_end; ++c) {
        ...
        if (connections > MAX_SIMULTANEOUS_CONNECTIONS) {
            ...
            send_bb_security(client, BB_LOGIN_ERROR_USERINUSE);
            return 4;
        }
    }
...
/* Send from data the number of bytes specified by len to a client. */
int send_packet(login_client *client, void *data, int len) {
    int total = 0, remaining = len;
    int bytes_sent;
    u_char *pkt = (u_char*) data;
    
    while (total < len) {
        bytes_sent = send(client->socket, pkt + total, remaining, 0);
        if (bytes_sent == -1) {
            perror("send");
            return -1;
        }
...
int receive_from_client(login_client *client) {
    ...
    if (client->recv_size < BB_HEADER_LEN) {
        ...
        bytes = recv(client->socket, client->recv_buffer, BB_HEADER_LEN - client->recv_size, 0);
        ...
        if ((select_result = select(fd_max + 1, &readfds, &writefds, &exceptfds, &timeout)) > 0) {
            ...
            for (c = clients.begin(), c_end = clients.end(); c != c_end; ++c) {
                ...
                if ((*c)->todc) {
                    ...
                    if ((*c)->send_size > 0) {
                        send_packet((*c), (*c)->send_buffer, (*c)->send_size);
                    }
                    close((*c)->socket);
                    clients.erase(c++);
                    destroy_client(*c);
